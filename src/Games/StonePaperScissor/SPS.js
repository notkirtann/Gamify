...
function Player({ name = "Player", score = 0, action = "rock" }) {
  return (
    <div className="player-sps">
      <div className="score-sps">{`${name}: ${score}`}</div>
      <div className="action-sps">
...
function ActionButton({ action = "rock", onActionSelected }) {
  return (
    <button className="round-btn-sps" onClick={() => onActionSelected(action)}>
...
function ShowWinner({winner = 0}) {
  ...
  return (
    <h2 className="sps-h2 ">{text[winner]}</h2>
  )
}
